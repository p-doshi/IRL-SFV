...
class MMfO_TD3:
    def __init__(
        ...
        self.critic = TrainState.create(
            ...
            params=critic_net.init(critic_rng_key, sample_state[None, ...], sample_action[None, ...]),
            target_params=critic_net.init(critic_rng_key, sample_state[None, ...], sample_action[None, ...]),
            tx=optax.chain(
        ...
        self.disc = TrainState.create(
            ...
            params=disc_net.init(disc_rng_key, sample_state[None, ...]),
            target_params=disc_net.init(disc_rng_key, sample_state[None, ...]),
        ...
        @chex.assert_max_traces(10)
        def _compute_episodic_gap(disc: TrainState,
                                  E_s: jnp.ndarray,
                                  Pi_s: jnp.ndarray):
            E_r = -disc.apply_fn(disc.params, E_s)
            Pi_r = -disc.apply_fn(disc.params, Pi_s)
            return E_r.mean() - Pi_r.mean()
        ...
        def _update_actor(
            ...
            def _loss_fn(params: Params):
                ...
                q = critic.apply_fn(critic.params, s, a)[0]
        ...
        def _update_critic(
            ...
            a_next = (
                actor.apply_fn(actor.target_params, s_next) + noise
            ).clip(-1, 1)

            target_qs = critic.apply_fn(critic.target_params, s_next, a_next)

            r = -disc.apply_fn(disc.params, s)
            target_q = r + gamma * jnp.clip(jnp.min(target_qs, axis=0), min=min_target, max=max_target)

            def _critic_loss_fn(
                params: Params,
            ):
                q1, q2 = critic.apply_fn(params, s, a)
        ...
        def _update_targets(
            ...
            new_critic = critic.replace(target_params=optax.incremental_update(
                critic.params,
    ...
    def get_episodic_gap(self,
        ...
        return self._compute_episodic_gap(self.disc,
                                          expert_states,
                                          policy_states)
    ...
    def update_disc(self,
                    learner_transitions,
                    expert_transitions):
        Pi_s = learner_transitions[0]
        E_s = expert_transitions[0]
        self.disc, disc_info = self._update_disc(
            self.disc,
            self.rng_key,
            Pi_s,
            E_s
    ...
    def update(
        ...
        self.critic = self.update_targets(self.critic)
...
if __name__ == "__main__":
    ...
    import time 
    cprint (f"Training state-only MM (TD3) on {cfg.env} for {cfg.steps} environment steps.", "green")
    for step in pbar:
