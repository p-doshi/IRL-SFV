...
class Args:
    ...
    """Flag if the temperature should be tuned or not"""
...
class SFM_SAC:
    def __init__(
        ...
        self.update_temperature = cfg.update_temperature
        ...
        def _update_actor(
            ...
            a_next, _, _ = actor.apply_fn(actor.params,
                                         s_next,
                                         a_next_key)

            next_psi_Q = psi_net.apply_fn(psi_net.target_params,
                                        s_next,
                                        a_next)[0]
            next_psi = next_psi_Q[:, :-1]
            ...
            def _loss_fn(params: Params,
                ...
                a, a_log_prob, _ = actor.apply_fn(params, s, a_key)
                psi_Q = psi_net.apply_fn(psi_net.params,
                                         s,
                                         a)[0]
                psi = psi_Q[:, :-1]
                Q = psi_Q[:, -1:]
                ...
                # loss_ent = (alpha * a_log_prob - Q).mean()
                ...
                return loss, {
                    ...
                    "E_SF_ema": E_SF_ema,
                    "entropy": a_log_prob,
                    "actor/log_prob": a_log_prob.mean(),
                    "actor/alpha": alpha,
                    "actor/Q_ent": Q.mean(),
                }
...
if __name__ == "__main__":
    ...
    if cfg.track:
        wandb.init(
            ...
            group=f"SFM_SAC_ds_minQ_v11/%s/%s_%s"
            % (
                cfg.env,
                cfg.phi_name,
                cfg.wandb_suffix
